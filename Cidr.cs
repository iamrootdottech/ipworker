...
namespace ipworker
{
    ...
    public static class CidrExtensions
    {
        ...
        public static List<Cidr> DeDupe(this List<Cidr> cidrs)
        {
            ...
            // 1. Sort by AddressFamily, then BaseAddress, then PrefixLength (shortest first)
            var sorted = cidrs
                .OrderBy(c => c.AddressFamily == AddressFamily.InterNetwork ? 0 : 1) // IPv4 before IPv6
                .ThenBy(c => c.BaseAddress, new IpComparer()) // custom comparer
                .ThenBy(c => c.PrefixLength)
                .ToList();
    ...
    {
        public static int CompareTo(this IPAddress a, IPAddress b)
        {
            if (a == null && b == null) return 0;
            if (a == null) return -1;
            if (b == null) return 1;

            // Normalize both to 16-byte arrays (IPv6 style)
            byte[] aBytes = a.GetAddressBytes();
            byte[] bBytes = b.GetAddressBytes();

            if (a.AddressFamily == AddressFamily.InterNetwork)
                aBytes = MapToIPv6Bytes(aBytes);

            if (b.AddressFamily == AddressFamily.InterNetwork)
                bBytes = MapToIPv6Bytes(bBytes);
        ...
        private static byte[] MapToIPv6Bytes(byte[] ipv4Bytes)
        {
            byte[] v6 = new byte[16];
            v6[10] = 0xFF;
            v6[11] = 0xFF;
            Buffer.BlockCopy(ipv4Bytes, 0, v6, 12, 4);
            return v6;
        }
    }
}